...
document.addEventListener('change', function (event) {
  if (!event.target.closest('variant-selects, variant-radios')) return;

  const badgeDataEl = document.getElementById('VariantBadgeData');
  if (!badgeDataEl) return;

  const badgeData = JSON.parse(badgeDataEl.textContent);

  const productInfo = event.target.closest('product-info');
  if (!productInfo) return;

  const variantId = productInfo.querySelector('[name="id"]')?.value;
  if (!variantId) return;

  const badgeText = badgeData[variantId];
  const badgeEl = document.querySelector('.variant-badge');
  if (!badgeEl) return;
